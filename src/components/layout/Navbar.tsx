          ...
          <div className="flex justify-between items-center h-16">
            ...
            <Link to="/" className="flex items-center space-x-2 hover:opacity-80 transition-opacity flex-shrink-0">
              <div className="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-blue-600 rounded-lg">
                <Stethoscope className="w-5 h-5 sm:w-6 sm:h-6 text-white fill-current" />
              </div>
              <div className="hidden sm:block">
                <h1 className="text-lg sm:text-xl font-bold text-blue-600">VitalPath</h1>
                <p className="text-xs text-gray-500">Medical Network</p>
              </div>
            </Link>
            ...
            {/* Navigation Items - Hidden when search is expanded */}
            {!isSearchExpanded && (
              <div className="flex items-center space-x-1">
                ...
                <div className="lg:hidden">
                  <Button
                    ...
                    className={`flex flex-col items-center px-2 sm:px-3 py-2 h-auto relative transition-colors text-gray-500 hover:text-gray-700`}
                  >
                    <Search className="w-6 h-6 sm:w-7 sm:h-7 mb-0 sm:mb-1" />
                    ...
                    {item.hasModal ? (
                      <div ref={item.path === '/messages' ? messagesRef : notificationsRef}>
                        <Button
                          ...
                          className={`flex flex-col items-center px-2 sm:px-3 py-2 h-auto relative transition-colors ${
                            isActive(item.path) || activeModal === (item.path === '/messages' ? 'messages' : 'notifications')
                              ? 'text-gray-900 hover:text-gray-900' 
                              : 'text-gray-500 hover:text-gray-700'
                          }`}
                        >
                          <item.icon className={`w-6 h-6 sm:w-7 sm:h-7 mb-0 sm:mb-1 ${
                            isActive(item.path) || activeModal === (item.path === '/messages' ? 'messages' : 'notifications') 
                              ? 'fill-current' : ''
                          }`} />
                      ...
                      <Link to={item.path}>
                        ...
                        >
                          <item.icon className={`w-6 h-6 sm:w-7 sm:h-7 mb-0 sm:mb-1 ${isActive(item.path) ? 'fill-current' : ''}`} />
                          <span className="hidden sm:block text-xs">{item.label}</span>
                          {item.badge && item.badge > 0 && (
                            <Badge className="absolute top-1 right-1 w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center p-0 text-xs">
                              {item.badge}
            ...
            <div className="flex items-center ml-2 sm:ml-4 flex-shrink-0">
              <Link to="/profile">
                <div className="flex items-center space-x-2 hover:bg-gray-50 rounded-lg p-1 sm:p-2 transition-colors cursor-pointer">
